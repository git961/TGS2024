...
{
#ifdef DEBUG
	DrawFormatString(location.x - 100, 50, 0xffffff, "%d", dust_timer);
...
void Enemy::DrawDust() const
{
	...
	for (int i = 0; i < 3; i++)
	{
		if (direction == false)
		{
			DrawRotaGraph((int)dust_x - move_x_img + i * 20, (int)dust_y + 20, size + i * 0.15, 0.0, dust_img, TRUE, FALSE);
		}
		else
		{
			// 画像
